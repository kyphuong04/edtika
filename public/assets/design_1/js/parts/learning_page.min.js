...
(function ($) {
  ...
  function handleTrackSpentTime() {
    ...
    setInterval(function () {
      var $activeItem = $('.js-content-tab-item.active');
      if ($activeItem.length) {
        var type = $activeItem.attr('data-type');
  ...
  $('body').on('change', '#sidebarNoteAttachInput', function () {
    ...
    if (files && files.length) {
      var $display = $('#sidebarNoteCurrentAttach');
  ...
  function initSidebarNoteEditor() {
    var $textarea = $('#sidebarNoteEditor');
    if (!$textarea.length) return;
    $textarea.summernote({
      toolbar: [['style', ['bold', 'italic', 'underline', 'strikethrough']], ['para', ['ul', 'ol', 'paragraph']]],
      height: 200,
      minHeight: 160,
      maxHeight: 320,
      focus: false,
  ...
  function loadSidebarPersonalNote(itemId, itemType) {
    var $panel = $('#sidebarNotePanel');
    if (!$panel.length) return;
    var $placeholder = $panel.find('.js-sidebar-note-placeholder');
    var $editorWrap = $panel.find('.js-sidebar-note-editor');
    ...
    $.get(path, function (result) {
      if (result && result.code === 200) {
        var $parsed = $('<div>').html(result.html);
        var noteContent = $parsed.find('textarea[name="details"]').val() || $parsed.find('textarea[name="details"]').text() || '';

        // Extract saved title from note content (stored as <h4 data-note-title="1">)
        var $temp = $('<div>').html(noteContent);
        var $titleEl = $temp.find('h4[data-note-title="1"]');
        if ($titleEl.length) {
  ...
  $('body').on('click', '.js-sidebar-save-note', function (e) {
    ...
    var itemId = $('#sidebarNoteItemId').val();
    var itemType = $('#sidebarNoteItemType').val();
    var noteTitle = $('#sidebarNoteTitleInput').val().trim();
    var noteContent = $('#sidebarNoteEditor').summernote('code');

    // Prepend title as a tagged element so it can be extracted on reload
    if (noteTitle) {
      noteContent = "<h4 data-note-title=\"1\">".concat(noteTitle, "</h4>").concat(noteContent);
    }
    if (!itemId || !itemType) {
    ...
    var path = "".concat(courseLearningUrl, "/personal-note/store");
    $.ajax({
      url: path,
      type: 'POST',
      data: {

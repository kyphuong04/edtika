...
/*!*********************************************!*\
  !*** ./resources/js/admin/parts/webinar.js ***!
  \*********************************************/
...
(function ($) {
  ...
  $('body').on('click', '.js-add-course-content-btn, .add-new-interactive-file-btn', function (e) {
    ...
    switch (type) {
      ...
      case 'interactive_quiz':
        var newInteractiveQuizForm = $('#newInteractiveQuizForm');
        newInteractiveQuizForm.find('.chapter-input').val(chapterId);
        html = newInteractiveQuizForm.html();
        html = html.replace(/record/g, key);
        $(contentTagId).prepend(html);
        break;
    }
  ...
  $('body').on('click', '.js-session-has-ended', function () {
    showToast('error', requestFailedLang, thisLiveHasEndedLang);
  });
  ...
  // Interactive Quiz
  $('body').on('click', '.save-interactive-quiz', function (e) {
    e.preventDefault();
    var $this = $(this);
    var form = $this.closest('.interactive-quiz-form');
    handleWebinarItemForm(form, $this);
  });
  $('body').on('click', '#add_interactive_multiple_question', function (e) {
    e.preventDefault();
    var quizId = $(this).attr('data-quiz-id');
    var addQuestionModal = '<div class="add-question-modal">';
    addQuestionModal += $('.multipleQuestionModal' + quizId).html();
    addQuestionModal += '</div>';
    var randomKey = randomString();
    addQuestionModal = addQuestionModal.replaceAll('record', randomKey);
    addQuestionModal = addQuestionModal.replaceAll('ans_tmp', randomKey);
    Swal.fire({
      html: addQuestionModal,
      showCancelButton: false,
      showConfirmButton: false,
      customClass: {
        content: 'p-0 text-left'
  ...
  $('body').on('click', '#add_interactive_descriptive_question', function (e) {
    e.preventDefault();
    var quizId = $(this).attr('data-quiz-id');
    var addQuestionModal = '<div class="add-question-modal">';
    addQuestionModal += $('.descriptiveQuestionModal' + quizId).html();
    addQuestionModal += '</div>';
    var randomKey = randomString();
    addQuestionModal = addQuestionModal.replaceAll('record', randomKey);
    Swal.fire({
  ...
  $('body').on('click', '.edit-interactive-question', function (e) {
    e.preventDefault();
    var $this = $(this);
    var questionId = $this.attr('data-question-id');
    var loadingSwl = Swal.fire({
      title: 'Loading...',
      showConfirmButton: false
    });
    $.get(adminPanelPrefix + '/interactive-quiz-questions/' + questionId + '/edit', function (result) {
      if (result && result.html) {
        var editQuestionModal = '<div class="add-question-modal">';
        editQuestionModal += result.html;
        editQuestionModal += '</div>';
        Swal.fire({
          html: editQuestionModal,
          showCancelButton: false,
          showConfirmButton: false,
          customClass: {
            content: 'p-0 text-left'
  ...
  $('body').on('click', '.add-answer-btn', function (e) {
    e.preventDefault();
    var $container = $(this).parent().parent().find('.add-answer-container');
    var mainRow = $container.find('.main-answer-row');
    var copy = mainRow.clone();
    copy.removeClass('main-answer-row');
    var removeBtn = copy.find('.answer-remove');
    if (removeBtn) {
      removeBtn.removeClass('d-none');
    }
    var copyHtml = copy.prop('innerHTML');
    var randomKey = randomString();
    copyHtml = copyHtml.replaceAll('ans_tmp', randomKey);
    copy.html(copyHtml);
    $container.append(copy);
  });
  ...
  * */
  ...
  $('body').on('change', '.js-video-demo-source', function (e) {
    ...
    var $fileInput = $('.js-video-demo-file-input');

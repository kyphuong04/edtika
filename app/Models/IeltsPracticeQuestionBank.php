<?php

namespace App\Models;

use Illuminate\Database\Eloquent\Model;

class IeltsPracticeQuestionBank extends Model
{
    protected $table = 'ielts_practice_question_bank';
    
    public $timestamps = false;
    
    protected $fillable = [
        'group_id',
        'question_order',
        'skill',
        'section_type',
        'question_type',
        'question_text',
        'instruction',
        'passage_text',
        'audio_file',
        'image_file',
        'timestamp_start',
        'timestamp_end',
        'answer_options',
        'correct_answer',
        'explanation',
        'points',
        'difficulty_level',
        'tags',
        'usage_count',
        'last_used_at',
        'practice_focus',  // Practice-specific
        'target_band',     // Practice-specific
        'created_by',
        'updated_by',
        'created_at',
        'updated_at',
    ];
    
    protected $casts = [
        'answer_options' => 'array',
        'tags' => 'array',
        'points' => 'decimal:1',
        'usage_count' => 'integer',
        'last_used_at' => 'integer',
        'created_at' => 'integer',
        'updated_at' => 'integer',
    ];
    
    // ============================================
    // Relationships
    // ============================================
    
    public function creator()
    {
        return $this->belongsTo(User::class, 'created_by');
    }
    
    public function updater()
    {
        return $this->belongsTo(User::class, 'updated_by');
    }
    
    public function testUsages()
    {
        return $this->hasMany(IeltsTestQuestionSource::class, 'bank_question_id')
            ->where('bank_type', 'practice');
    }
    
    public function group()
    {
        return $this->belongsTo(IeltsQuestionGroup::class, 'group_id');
    }
    
    // ============================================
    // Scopes (Same as Mock + Practice-specific)
    // ============================================
    
    public function scopeBySkill($query, $skill)
    {
        return $query->where('skill', $skill);
    }
    
    public function scopeByType($query, $type)
    {
        return $query->where('question_type', $type);
    }
    
    public function scopeByDifficulty($query, $difficulty)
    {
        return $query->where('difficulty_level', $difficulty);
    }
    
    public function scopeSearch($query, $search)
    {
        return $query->where(function($q) use ($search) {
            $q->where('question_text', 'LIKE', "%{$search}%")
              ->orWhere('passage_text', 'LIKE', "%{$search}%")
              ->orWhere('instruction', 'LIKE', "%{$search}%");
        });
    }
    
    public function scopeByTag($query, $tag)
    {
        return $query->whereRaw('JSON_CONTAINS(tags, ?)', [json_encode($tag)]);
    }
    
    /**
     * Filter by target band (Practice-specific)
     */
    public function scopeByTargetBand($query, $band)
    {
        return $query->where('target_band', 'LIKE', "%{$band}%");
    }
    
    /**
     * Filter by practice focus (Practice-specific)
     */
    public function scopeByPracticeFocus($query, $focus)
    {
        return $query->where('practice_focus', $focus);
    }
    
    public function scopeMostUsed($query)
    {
        return $query->orderBy('usage_count', 'desc');
    }
    
    public function scopeLeastUsed($query)
    {
        return $query->orderBy('usage_count', 'asc');
    }
    
    public function scopeExcludeRecentlyUsed($query, $days = 30)
    {
        $cutoff = time() - ($days * 24 * 60 * 60);
        return $query->where(function($q) use ($cutoff) {
            $q->whereNull('last_used_at')
              ->orWhere('last_used_at', '<', $cutoff);
        });
    }
    
    public function scopeNewest($query)
    {
        return $query->orderBy('created_at', 'desc');
    }
    
    // ============================================
    // Accessors (Same as Mock)
    // ============================================
    
    public function getSkillLabelAttribute()
    {
        $icons = [
            'listening' => '🎧',
            'reading' => '📖',
            'writing' => '✍️',
            'speaking' => '🗣️',
        ];
        
        return ($icons[$this->skill] ?? '') . ' ' . ucfirst($this->skill);
    }
    
    public function getDifficultyBadgeAttribute()
    {
        $badges = [
            'beginner' => 'success',
            'intermediate' => 'warning',
            'advanced' => 'danger',
        ];
        
        return $badges[$this->difficulty_level] ?? 'secondary';
    }
    
    public function getTypeNameAttribute()
    {
        return str_replace('_', ' ', ucwords($this->question_type, '_'));
    }
    
    public function getHasOptionsAttribute()
    {
        return in_array($this->question_type, [
            'multiple_choice',
            'matching_information',
            'matching_features',
            'matching_headings',
            'matching_sentence_endings',
            'yes_no_not_given',
            'true_false_not_given',
        ]);
    }
    
    public function getAutoGradableAttribute()
    {
        return !in_array($this->question_type, [
            'essay',
            'letter_writing',
            'graph_description',
            'report_writing',
            'speaking_prompt',
        ]);
    }
    
    // ============================================
    // Methods
    // ============================================
    
    public function incrementUsage()
    {
        $this->increment('usage_count');
        $this->update(['last_used_at' => time()]);
    }
    
    public function getTagBadges()
    {
        if (empty($this->tags)) {
            return '';
        }
        
        $html = '';
        foreach ($this->tags as $tag) {
            $html .= '<span class="badge badge-light mr-1">' . e($tag) . '</span>';
        }
        
        return $html;
    }
    
    public function duplicate()
    {
        $clone = $this->replicate();
        $clone->question_text = $this->question_text . ' (Copy)';
        $clone->usage_count = 0;
        $clone->last_used_at = null;
        $clone->created_at = time();
        $clone->updated_at = time();
        $clone->save();
        
        return $clone;
    }
}

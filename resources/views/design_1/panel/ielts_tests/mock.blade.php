...
.wf2-skill-mini strong { font-size: 16px; color: var(--wf2-ink); }
...
/* Placeholder cho hashtag loại câu hỏi — sẽ bổ sung ở bước sau theo yêu
   cầu (tạm thời KHÔNG hiển thị). Giữ hook .wf2-practice-card__tags sẵn
   trong markup để dễ chèn lại sau này. */
.wf2-practice-card__tags { display: none; }
...
.wf2-filter-apply:hover { background: var(--wf2-accent-hover); }
.wf2-filter-reset {
    width: 100%;
    padding: 12px;
...
/* Bộ lọc "Độ khó" tạm ẩn — chưa có field dữ liệu tương ứng (xem ghi chú
   trong Controller::indexMock()). Bỏ class này khi bổ sung field. */
.wf2-filter-group.wf2-hidden { display: none; }
...
@php
    ...
    // Nhãn "Passage X / Part X / Task X" theo từng skill, dùng section_number
    // của section đại diện. Bài "Full đề" (nhiều section cùng skill) hiển
    // thị nhãn khác vì không gắn với 1 passage/part cụ thể.
    $partLabelFor = function ($test) {
        if (empty($test->primary_skill) || empty($test->display_section)) {
            return null;
        }
        if ($test->is_full_test) {
            return 'Full đề';
        }
        $number = $test->display_section->section_number ?? null;
        $labels = [
            'reading'   => 'Passage',
            'listening' => 'Part',
            'writing'   => 'Task',
        ...
        $prefix = $labels[$test->primary_skill] ?? 'Part';
    ...
    <div class="row">
        ...
        <div class="col-12 col-lg-9 mb-20">
            ...
            <div class="wf2-search-wrap">
                <div class="wf2-search">
                    {!! $ico('search', 18) !!}
                    <input type="text" id="wf2Search" placeholder="Tìm kiếm bài luyện IELTS...">
                </div>
            </div>
                    ...
                    @else
                        @foreach($practiceTests as $test)
                            ...
                            <div class="wf2-practice-card wf2-card"
                                 ...
                                 data-passage="{{ optional($test->display_section)->section_number }}">
                                ...
                                {{-- Hashtag loại câu hỏi — tạm ẩn theo yêu cầu, sẽ bổ sung sau. --}}
                                <div class="wf2-practice-card__tags"></div>
        ...
        <div class="col-12 col-lg-3">
            <div class="wf2-filter-panel">
                ...
                {{-- Độ khó — TẠM ẨN, chưa có field difficulty_level trên
                     ielts_tests. Bỏ class "wf2-hidden" khi có dữ liệu. --}}
                <div class="wf2-filter-group wf2-hidden collapsed">
                    ...
                    <div class="wf2-filter-group__body">
                        <label class="wf2-filter-check"><input type="checkbox"> Level 1</label>
                        <label class="wf2-filter-check"><input type="checkbox"> Level 2</label>
                        <label class="wf2-filter-check"><input type="checkbox"> Level 3</label>
                    </div>
                </div>
                ...
                <div class="wf2-filter-group">
                    ...
                    <div class="wf2-filter-group__body">
                        <div class="wf2-filter-sub-label">Bài lẻ</div>
                        <label class="wf2-filter-check">
                            <input type="checkbox" class="wf2-filter-skill" data-skill="reading" data-mode="single" data-passage="1"> Passage 1
                        </label>
                        <label class="wf2-filter-check">
                            <input type="checkbox" class="wf2-filter-skill" data-skill="reading" data-mode="single" data-passage="2"> Passage 2
                        </label>
                        <label class="wf2-filter-check">
                            <input type="checkbox" class="wf2-filter-skill" data-skill="reading" data-mode="single" data-passage="3"> Passage 3
                        </label>
                ...
                <div class="wf2-filter-group">
                    ...
                    <div class="wf2-filter-group__body">
                        <label class="wf2-filter-check">
                            <input type="checkbox" class="wf2-filter-skill" data-skill="listening" data-mode="single"> Bài lẻ
                        </label>
                        <label class="wf2-filter-check">
                ...
                <div class="wf2-filter-group">
                    ...
                    <div class="wf2-filter-group__body">
                        <label class="wf2-filter-check">
                            <input type="checkbox" class="wf2-filter-skill" data-skill="writing" data-mode="single"> Bài lẻ
                        </label>
                        <label class="wf2-filter-check">
                ...
                <div class="wf2-filter-group">
                    ...
                    <div class="wf2-filter-group__body">
                        <label class="wf2-filter-check">
                            <input type="checkbox" class="wf2-filter-skill" data-skill="speaking" data-mode="single"> Bài lẻ
                        </label>
                        <label class="wf2-filter-check">
                ...
                <button type="button" id="wf2FilterApply" class="wf2-filter-apply">Áp dụng bộ lọc</button>
...
(function () {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', wf2ApplySearch);
    }

    // ---------- Filter sidebar (Áp dụng / Đặt lại) ----------
    function wf2ApplyFilters() {
        ...
        // Gom điều kiện theo skill: { reading: { full: bool, single: bool, passages: Set }, ... }
        var skillFilters = {};
        document.querySelectorAll('.wf2-filter-skill:checked').forEach(function (el) {
            ...
            if (!skillFilters[skill]) {
                skillFilters[skill] = { full: false, single: false, passages: new Set() };
            }
            if (mode === 'full') skillFilters[skill].full = true;
            if (mode === 'single') {
                skillFilters[skill].single = true;
                if (passage) skillFilters[skill].passages.add(passage);
            }
        });
    ...
    function wf2ResetFilters() {
        document.querySelectorAll('.wf2-filter-skill, .wf2-filter-status').forEach(function (el) { el.checked = false; });
    ...
    var applyBtn = document.getElementById('wf2FilterApply');
    var resetBtn = document.getElementById('wf2FilterReset');
    if (applyBtn) applyBtn.addEventListener('click', wf2ApplyFilters);

        ...
        <div class="d-flex gap-8">
            ...
            <div class="btn-group">
                <a href="?{{ http_build_query(array_merge(request()->except('view'), ['view' => 'table'])) }}" 
                   class="btn btn-sm btn-{{ $viewMode === 'table' ? 'primary' : 'outline-secondary' }}">
                    <x-iconsax-bul-menu class="icons" width="16px" height="16px"/>
                </a>
                <a href="?{{ http_build_query(array_merge(request()->except('view'), ['view' => 'card'])) }}" 
                   class="btn btn-sm btn-{{ $viewMode === 'card' ? 'primary' : 'outline-secondary' }}">
    ...
    <div class="row">
        ...
        <div class="col-lg-3">
            <div class="bg-white rounded-24 p-16 sticky-sidebar">
                ...
                <a href="?" class="topic-item d-flex align-items-center justify-content-between p-12 rounded-12 mb-4 {{ !request('topic') ? 'active' : '' }}">
                ...
                @if($topics && count($topics) > 0)
                    @foreach(array_slice($topics, 0, 15) as $topic => $count)
                        <a href="?topic={{ urlencode($topic) }}&view={{ $viewMode }}" 
                           class="topic-item d-flex align-items-center justify-content-between p-12 rounded-12 mb-4 {{ request('topic') === $topic ? 'active' : '' }}">
        ...
        <div class="col-lg-9">
                        ...
                        @foreach($questions as $question)
                            <div class="question-card">
                                ...
                                <div class="question-actions">
                                    <a href="{{ route('panel.question_bank.edit', ['mock', $question->id]) }}" class="btn-action btn-edit">
                                        <x-iconsax-bul-edit class="icons" width="16px" height="16px"/>
                                    </a>
                                    <a href="{{ route('panel.question_bank.delete', ['mock', $question->id]) }}" 
                                    ...
                                    <tr>
                                        ...
                                        <td class="question-cell">
                                            ...
                                            <div class="question-info">
                                                <span class="info-badge">{{ $question->type_name }}</span>
                                                @if($question->tags)
                                        ...
                                        <td class="actions-cell">
                                            <div class="table-actions">
                                                <a href="{{ route('panel.question_bank.edit', ['mock', $question->id]) }}" class="btn-action-sm">
            ...
            @else
                ...
                <div class="empty-state">
                    ...
                    <a href="{{ route('panel.question_bank.create', ['bank_type' => 'mock']) }}" class="btn btn-primary">
                        <x-iconsax-bul-add class="icons mr-8" width="16px" height="16px"/>Add Mock Question
                    </a>
                </div>
            @endif
        </div>

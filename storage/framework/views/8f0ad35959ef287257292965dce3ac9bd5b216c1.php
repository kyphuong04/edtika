...
<?php echo $__env->make('design_1.panel.ielts_tests.partials.speaking_mic_check', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
...
<div class="speaking-container" style="display: flex; height: 100%; background: #f5f5f5;">
    ...
    <div class="speaking-question-panel" style="flex: 1; background: #fff; overflow-y: auto; padding: 30px 36px; border-right: 1px solid #ddd;">
        ...
        <?php if($partNum == 2): ?>
            <div class="cue-card" style="background: linear-gradient(145deg, #fffbeb 0%, #fef3c7 100%); border: 2px solid #f59e0b; border-radius: 12px; padding: 24px; margin-bottom: 20px; box-shadow: 0 4px 12px rgba(245, 158, 11, 0.15);">
                ...
                <div style="font-size: 17px; color: #78350f; line-height: 1.7; font-weight: 500; margin-bottom: 16px;">
                    <?php echo nl2br(e($taskText)); ?>

                </div>
                
                <?php if(!empty($cueCardPoints)): ?>
                    <div style="background: rgba(255,255,255,0.7); border-radius: 8px; padding: 16px; margin-top: 16px;">
                        ...
                        <ul style="margin: 0; padding-left: 24px; color: #78350f; font-size: 15px; line-height: 2;">
                            <?php $__currentLoopData = $cueCardPoints; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $point): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php if(trim($point)): ?>
        ...
        <?php if(!empty($task->image_url)): ?>
            <div style="margin-top: 16px;">
                <img src="<?php echo e($task->image_url); ?>" alt="Task image" style="max-width: 100%; border: 1px solid #e5e7eb; border-radius: 8px;">
            </div>
        <?php endif; ?>
    ...
    <div class="speaking-record-panel" style="flex: 1; display: flex; flex-direction: column; padding: 40px; background: linear-gradient(145deg, #f9fafb 0%, #f3f4f6 100%); align-items: center; justify-content: center;">
        ...
        <div id="speakingTimerSection" style="text-align: center; margin-bottom: 32px;">
            <?php if($prepTime > 0): ?>
                <div id="prepPhaseSection">
                    ...
                    <div id="prepTimerDisplay" style="font-size: 56px; font-weight: 700; color: #3b82f6; font-variant-numeric: tabular-nums; text-shadow: 0 2px 4px rgba(59, 130, 246, 0.2);">
                        <?php echo e(sprintf('%02d:%02d', floor($prepTime/60), $prepTime%60)); ?>
            ...
            <div id="speakPhaseSection" style="<?php echo e($prepTime > 0 ? 'display: none;' : ''); ?>">
                ...
                <div id="speakTimerDisplay" style="font-size: 56px; font-weight: 700; color: #1f2937; font-variant-numeric: tabular-nums;">
                    <?php echo e(sprintf('%02d:%02d', floor($speakTime/60), $speakTime%60)); ?>
        ...
        <div id="speakingControls" style="display: flex; gap: 14px; justify-content: center; flex-wrap: wrap;">
            ...
            <button type="button" id="btnStartRecording" onclick="SpeakingTest.toggleRecording()" style="padding: 16px 32px; font-size: 16px; font-weight: 600; border-radius: 12px; border: none; cursor: pointer; background: linear-gradient(135deg, #E31837 0%, #be123c 100%); color: white; display: <?php echo e($prepTime > 0 ? 'none' : 'flex'); ?>; align-items: center; gap: 10px; box-shadow: 0 4px 14px rgba(227, 24, 55, 0.35); transition: all 0.2s;">
        ...
        <input type="hidden" id="speakingAnswerInput" name="speaking_answer" data-question-id="<?php echo e($task->id ?? 0); ?>" value="<?php echo e($savedAnswer); ?>">
    </div>
</div>
...
const SpeakingTest = {
    ...
    audioStream: null,
    audioChunks: [],
    prepTimeRemaining: <?php echo e($prepTime); ?>,
    speakTimeRemaining: <?php echo e($speakTime); ?>,
    totalPrepTime: <?php echo e($prepTime); ?>,
    totalSpeakTime: <?php echo e($speakTime); ?>,
    timerInterval: null,
    isRecording: false,
    hasRecording: false,
    questionId: <?php echo e($task->id ?? 0); ?>,
...
<?php /**PATH C:\xampp\htdocs\edtika\resources\views/design_1/panel/ielts_tests/partials/idp_speaking_complete.blade.php ENDPATH**/ ?>